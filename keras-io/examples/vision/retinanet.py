...
def random_flip_horizontal(image, boxes):
    ...
    def f2(image, boxes):
        return image, boxes
    image, boxes = tf.cond(tf.less(0.5, tf.random.uniform(())), lambda: f1(image, boxes), lambda: f2(image, boxes))
...
def resize_and_pad_image(
    ...
    def f1(ratio, max_side, image_shape): 
        ratio = max_side / tf.reduce_max(image_shape)
        return ratio
    def f2(ratio, max_side, image_shape):
        return ratio
    ratio = tf.cond(tf.greater( tf.reduce_max(image_shape), max_side), lambda: f1(ratio, max_side, image_shape), lambda: f2(ratio, max_side, image_shape))
